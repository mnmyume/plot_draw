...
class MetricsBoxPlotter(BasePlotter):
    ...
    def plot(self) -> None:
        ...
        self._finalize_and_save(
            ...
            title=f"Metrics results:{metric_key}",
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--plot",
        choices=["metrics", "constraints", "both"],
        default="metrics",
    ...
    parser.add_argument(
        ...
        default="overall",
        help="Difficulty level for constraints plot (only used with --plot constraints or both)",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
    ...
    if args.plot in ("constraints", "both"):
